...
const ROOT_BORDER = 2;
const ICON_W = 18;
const ACTIVE_LEFT_PAD = 8;
const ACTIVE_RIGHT_PAD = 12;
const LABEL_GAP = 6;
...
export function ExpandableTabs({
  ...
  const activeExtraWidth = active ? getActiveTabWidth(active) - TAB_W : 0;
  const activeBarWidth = closedSize.width + activeExtraWidth;
  const hasContent = Boolean(active?.content);
  const openSize = size && hasContent
    ? {
        width: Math.max(size.width + ROOT_BORDER, activeBarWidth),
        height: Math.max(size.height + ROOT_BORDER, closedSize.height),
      }
    : { ...closedSize, width: activeBarWidth };
    ...
    <>
      ...
      >
        ...
        <div
          ...
          className={cn(
            "absolute bottom-0 left-0 z-20 flex w-full items-center justify-center gap-1 p-1.5",
            classNames?.bar,
          )}
          style={{ height: BAR_H }}
        >
          {items.map((item) => {
            ...
            return (
              <motion.button
                ...
                className={cn(
                  "relative isolate flex h-8 min-w-7 shrink-0 items-center justify-center overflow-hidden rounded-[16px] px-2 text-sm font-medium outline-none",
                  "focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background",
                  active && isActive && "min-w-0 justify-start pl-2 pr-3.5",
                  isActive
                    ? "text-foreground"
