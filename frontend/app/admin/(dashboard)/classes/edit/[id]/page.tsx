...
export default function EditClassPage() {
  ...
  const hasInput = formData.class_title || formData.class_description || formData.batch || formData.class_price || previewUrl || currentImageUrl;
  ...
  useEffect(() => {
    ...
    const fetchClass = async () => { 
      try {
        const res = await fetch(`/api/admin/classes?class_id=${classId}`,{
        ...
        if (data.length > 0) {
          ...
          setFormData({
            ...
            class_price: cls.class_price.toString(),
            class_type: cls.class_type || "theory",
            renew_type: cls.renew_type || "30days",
            batch: cls.batch,
            class_code: cls.class_code || "",
          });
          setCurrentImageUrl(cls.class_imageurl);
          setPreviewUrl(cls.class_imageurl);
        }
        setLoading(false);
      } catch (err) {
        console.error(err);
  ...
  if (loading) {
    return (
      <div className="p-6">
        <h1 className="text-4xl font-bold mb-8">Edit Class</h1>
          ...
          {!hasInput ? (
            ...
            <Card className="relative w-full max-w-md h-[420px] overflow-hidden shadow-2xl flex flex-col justify-between">
              ...
              <div className="absolute top-3 left-3 z-30">
                <Chip size="sm" color="warning" variant="solid" className="font-bold uppercase tracking-wider text-[10px] text-black">
                  Updated
                </Chip>
              </div>
